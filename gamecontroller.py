...
AXIS_LEFT_X       = 1  # Rotate
AXIS_LEFT_Y       = 0  # Bend
AXIS_LEFT_TRIGGER = 5  
AXIS_RIGHT_X      = 4  # Rotate
AXIS_RIGHT_Y      = 3  # Bend
...
def joystick_thread_func(
    ...
    while not stop_event.is_set():
        ...
        if lb:
            ...
            if 3 in node_ids:
                joint_positions[3] += (rx * VELOCITY_SCALING * dt)  
            ...
            if 4 in node_ids:
                joint_positions[4] -= (ry * VELOCITY_SCALING * dt)  
            ...
            if not wrist_mode:
                ...
                if wrist_ctrl and (5 in node_ids) and (6 in node_ids):
                    # Create swapped variables for wrist control (swapping bend and rotate)
                    wrist_bend_input   = raw_rotate 
                    wrist_rotate_input = raw_bend   

                    new_bend   = wrist_ctrl.bend_pos   + (wrist_bend_input   * VELOCITY_SCALING * FOREARM_VELOCITY_SCALING * dt)
                    new_rotate = wrist_ctrl.rotate_pos + (wrist_rotate_input * VELOCITY_SCALING * FOREARM_VELOCITY_SCALING * dt)
            ...
            if 7 in node_ids:
                ...
                if raw_lt > 0: new_pos -= raw_lt * VELOCITY_SCALING * GRIPPER_SCALING * dt
                # Pressing right trigger => move positive
                if raw_rt > 0: new_pos += raw_rt * VELOCITY_SCALING * GRIPPER_SCALING * dt
